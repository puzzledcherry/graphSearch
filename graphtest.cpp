...
void testGraph0NotDirected() {
  ...
  cout << "non-directional edges: " << endl;
  cout << "A: ";
  cout << g.getEdgesAsString("A") << endl;
  cout << "B: ";
  cout << g.getEdgesAsString("B") << endl;
  cout << "C: ";
  cout << g.getEdgesAsString("C") << endl;
  ...
  cout << endl;
  cout << "should be: BAC" << endl;

  globalSS.str("");
  g.dfs("C", vertexPrinter);
  assert(globalSS.str() == "CAB" && "starting from C");
  cout << endl;
  cout << "should be: CAB" << endl;
  ...
  assert(map2string(weights).empty() && "Dijkstra(C) weights");
  assert(map2string(previous).empty() && "Dijkstra(C) previous");
  cout << "non-directional tests passed" << endl;
  cout << endl;
...
void testGraph1() {
  ...
  assert(map2string(previous) == "[B:A][C:B][D:C][E:D][F:E][G:H][H:A]" &&
         "Dijkstra(B) previous");
  cout << "graph 1 tests passed" << endl;
  cout << endl;
}

void testDirectionalGraph() {
  cout << "--------testDirectionalGraph--------" << endl;
  Graph g;
  ...
  assert(g.connect("a", "b", 10) && "connect a b");
  assert(g.connect("b", "c", 2) && "connect b c");
  assert(g.connect("e", "b", 3) && "connect e b");
  assert(g.connect("a", "d", 6) && "connect a b");

  assert(!g.connect("e", "b") && "connect e b");
  assert(!g.connect("c", "c", 10) && "connect c c");

  assert(g.edgesSize() == 4);
  cout << "directional graph tests passed" << endl;
  cout << endl;
}

void testNonDirectionalGraph() {
  cout << "--------testNonDirectionalGraph--------" << endl;
  bool isDirectional = false;
  Graph g(isDirectional);
  ...
  cout << g.getEdgesAsString("a") << endl;
  assert(g.disconnect("a", "d") && "disconnect a d");
  cout << g.getEdgesAsString("a") << endl;
  assert(g.disconnect("a", "b") && "disconnect a b");
  assert(!g.disconnect("a", "e") && "disconnect a e");

  cout << "made it here" << endl;

  assert(g.edgesSize() == 2);

  cout << "non directional graph tests passed" << endl;
  cout << endl;
}
