...
class Graph {
private:
  // store the graph in this map
  map<string, vector<vector<string>>> theGraph;
  ...
  // helper methods
  // num edges helpers
  int directionalEdgesCounter() const;
  int nondirectionalEdgesCounter() const;

  // connection helpers
  bool directionalConnection(const string &from, const string &to,
                             int weight = 0);
  bool nondirectionalConnection(const string &from, const string &to,
                                int weight = 0);

  // disconnection helpers
  bool directionalDisconnection(const string &from, const string &to);
  bool nondirectionalDisconnect(const string &from, const string &to);

  // DFS helper
  string dfsHelper(string results, map<string, bool> &visited,
                   const string &startLabel);

  // BFS helper
  string bfsHelper(string results, map<string, bool> &visited,
                   const string &startLabel);
